...
JNIEXPORT jlongArray JNICALL Java_PathfindingVisualizer_findPath
  (JNIEnv* env, jobject obj, jint startX, jint startY, jint endX, jint endY, jobjectArray jMap) {
    // Convert Java boolean[][] to C++ vector<vector<bool>>
    jsize rows = env->GetArrayLength(jMap);
    jsize cols = env->GetArrayLength((jbooleanArray)env->GetObjectArrayElement(jMap, 0));
    
    std::vector<std::vector<bool>> map(rows, std::vector<bool>(cols));
    
    for (jsize i = 0; i < rows; i++) {
        jbooleanArray row = (jbooleanArray)env->GetObjectArrayElement(jMap, i);
...
JNIEXPORT jobjectArray JNICALL Java_PathfindingVisualizer_findPathWithSteps
  (JNIEnv* env, jobject obj, jint startX, jint startY, jint endX, jint endY, jobjectArray jMap) {
    // Convert Java boolean[][] to C++ vector<vector<bool>>
    jsize rows = env->GetArrayLength(jMap);
    jsize cols = env->GetArrayLength((jbooleanArray)env->GetObjectArrayElement(jMap, 0));
